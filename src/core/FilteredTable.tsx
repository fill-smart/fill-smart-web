...
import { IFilterCriteria, FilterTypesEnum, IAndFilterCriteria, IPropertyFilterCriterion } from "./filters";
...
export type QueryCriteria = { pagination: { current: number, pageSize: number }, sort: Array<{ property: string, descending: boolean }>, filter: IFilterCriteria | undefined }

export const formatFilters = (formValues: { [key: string]: { type: string, value: any } }): IAndFilterCriteria => {
    return {
        and: Object.keys(formValues).map(key => [key, formValues[key].type, formValues[key].value] as [string, string, any]).map(([property, type, value]) => ({
            property,
            type,
            value
...
export const FilteredTable = ({
    ...
    const [tableState, setTableState] = useState<{ pagination: { current: number, pageSize: number }, sort: Array<{ property: string, descending: boolean }>, filter: IFilterCriteria | undefined }>({
        pagination: {
            current: 1,
            pageSize: 10
        },
        sort: defaultSorter,
        filter: { and: defaultFilters.and.concat((showDeleted ? [] : [{ property: "deleted", type: FilterTypesEnum.Equals, value: false }])) }
    })

    const filteredColumns = columns.map(col => {
        const c = col;
        const isFiltered = col.filterDropdown && (tableState?.filter as IAndFilterCriteria)?.and?.find(f => {
        ...
        c.filterIcon = () => {
            ...
            return <Icon type="filter" style={{ color: isFiltered ? "#1890ff" : "#bfbfbf" }} />
        }
    ...
    const removeFilter = (index: number) => {
        setTableState({ ...tableState, ...{ filter: { and: (tableState.filter as IAndFilterCriteria).and.filter((c, i) => i !== index + 1) } } })
    }

    const handleTableChange = (pagination, filters: { [key: string]: Array<any> }, sorter) => {
        let sort: {
        ...
        if (sorter.order) {
            sort = {
                property: sorter.columnKey, descending: sorter.order !== "ascend"
            }
        }
        let filter: IFilterCriteria | undefined = tableState.filter ? { ...tableState.filter, ...otherFilters } : otherFilters;
        if (Object.keys(filters).length > 0) {
            Object.keys(filters).map(p => [p, filters[p]] as [string, Array<any>]).map(([prop, values]: [string, Array<any>]) => {
                if (!filter) {
                    filter = { and: [] }
                }
                const condition = {
                    property: prop,
                    type: values[0],
                    value: (values[0] as FilterTypesEnum) !== FilterTypesEnum.Between ? values[1] : {
                        from: values[1],
                        to: values[2]
                    }
                };
                if (condition && condition.type && condition.value) {
                    filter = { and: [...(filter as IAndFilterCriteria)?.and.filter(f => (f as IPropertyFilterCriterion).property != condition.property)] };
                    (filter as IAndFilterCriteria)?.and.push(condition);
                }
            })
        }
        setTableState({ pagination: { ...pagination, ...{ current: pagination.current } }, sort: sort ? [sort] : defaultSorter, filter: filter })
    };

    useEffect(() => {
        const withOtherFiltersState = { ...tableState, filter: { and: [...(tableState.filter as IAndFilterCriteria)?.and ?? [], ...otherFilters && otherFilters.and ? otherFilters.and : []] } }
