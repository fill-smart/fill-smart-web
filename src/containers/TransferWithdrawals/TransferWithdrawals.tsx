...
import useAcceptOrRejectTransferWithdrawal, { IConfirTransactionModel } from "../../hooks/use-accept-or-reject-transfer.hook";
import { EnterTransferCode } from "./EnterTransferCode";
import useTransferWithdrawals, { TransferWithdrawalRecord } from "../../hooks/use-transfer-withdrawals.hook";
...
const Transfers = () => {
    ...
    const { accept, reject } = useAcceptOrRejectTransferWithdrawal(() => { refetch(); refetchTransfer() });
    const [tableCriteriaTransferWithdrawals, setTableCriteriaTransferWithdrawals] = useState<QueryCriteria>({
        pagination: {
            current: 1,
            pageSize: 10
        },
    ...
    const { transferWithdrawals, loading: loadingTransfer, refetch: refetchTransfer, total: totalTransfer } = useTransferWithdrawals(tableCriteriaTransferWithdrawals);
    ...
    const { pendingTransferWithdrawals, loading: loadingPending, refetch, total } = usePendingTransferWithdrawals(tableCriteria);
    ...
    const generateExcel = async () => {
        ...
        sheet.addRows(transferWithdrawals!.map(o => [
            o.code ?? "-",
            o.authorized ? "Autorizada" : "Rechazada",
            moment(o.stamp).format("DD/MM/YYYY HH:mm"),
            o.withdrawal.wallet.customer.firstName + " " + o.withdrawal.wallet.customer.lastName,
            o.withdrawal.wallet.customer.documentNumber,
        ...
        await workbook.xlsx.writeBuffer().then(data => {
            ...
            FileSaver.saveAs(blob, "fillsmart_transferencias_" + moment().format("DD-MM-YYYY") + ".xlsx");
        });;
    };
    ...
    const [transferWithdrawalsOtherFilters, setTransferWithdrawalsOtherFilters] = useState<IAndFilterCriteria | undefined>()

    if (loadingTransfer || loadingPending) return <LoaderComponent />;
    return (
            ...
            <div style={styles.widgetPageStyle}>
                        ...
                        <IsoWidgetsWrapper>
                            <IsoWidgetBox>
                                ...
                                <FilteredTable
                                    ...
                                    pagination={{ ...tableCriteriaTransferWithdrawals.pagination, ...{ totalTransfer } }}
                                    onCriteriaChange={setTableCriteriaTransferWithdrawals}
                                    otherFilters={transferWithdrawalsOtherFilters}
                                />
                            </IsoWidgetBox>
                        </IsoWidgetsWrapper>
                    </Col>
                ...
                <EnterTransferCode
                    ...
                    onFinished={(v: IConfirTransactionModel) => {
                        accept(acceptedTransferId, v.code, v.voucher.fileList.map(f => f.originFileObj));
