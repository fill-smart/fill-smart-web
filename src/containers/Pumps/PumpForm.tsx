...
const PumpFormFields = forwardRef<
    FormComponentProps,
    FormProps<Partial<IPumpModel>>
>(({ form, onSubmit, initialValue }, ref) => {
    ...
    return (
        <Form onSubmit={handleSubmit}>
            <FormItem {...formItemLayout} label="Numero Externo" hasFeedback>
                {getFieldDecorator("externalId", {
                    rules: [
                        {
                            required: true,
                            message: "Debe ingresar el numero externo"
                        }
                    ]
                })(<Input />)}
            </FormItem>
...
const PumpForm = Form.create<FormProps<Partial<IPumpModel>>>()(
    PumpFormFields
);
