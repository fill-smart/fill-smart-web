...
const ContactEmailFormFields = forwardRef<
    FormComponentProps,
...
>(({ form, onSubmit, initialValue }, ref) => {
    ...
    return (
        <Form onSubmit={handleSubmit}>
            <FormItem
                {...formItemLayout}
                label="Emails de contacto (separados por coma)"
                hasFeedback
            >
                    ...
                    rules: [
                        {
                            validator(rule, value: string, callback){
                                if (!value) {
                                    callback("Debe ingresar al menos un email");
                                }
                                else {
                                    const emails: string[] = value.split(",").map(email => email.trim());
                                    const validEmails: string[] = emails.filter(email => (/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(email)));
                                    if (validEmails.length !== emails.length) {
                                        callback("Error en el formato de los emails");
                                    }
                                    callback();
                                }
                            }
...
const ContactEmailForm = Form.create<FormProps<Partial<IParameterModel>>>()(
    ContactEmailFormFields
);
